...
```bash
for i in `ls gbks/ | sed 's/.gbk//g'`;do 
    anvi-script-process-genbank -i gbks/${i}.gbk -O ${i}; 
done
...
```bash
cd processed_gbks
for i in `ls *.fa | sed 's/-contigs.fa//g'`;do 
    anvi-gen-contigs-database -f ${i}-contigs.fa -n ${i} --external-gene-calls ${i}-external-gene-calls.txt -o ${i}.db; 
done
